...
public class BinarySerializer {
	...
	public interface PrimGroupWriterInterface {
		/**
		 * This callback is invoked on each group that is going into the
		 * fileblock in order to give it a chance to add to the stringtable pool
		 * of strings.
		 */
		public void addStringsToStringtable(StringTable stable);

		/**
		 * This callback is invoked to request that the primgroup serialize
		 * itself into the given protocol buffer object.
		 */
		public Osmformat.PrimitiveGroup serialize(BinarySerializer serializer);
	}

	/**
	 * Set the granularity (precision of lat/lon, measured in unites of
	 * nanodegrees.
	 */
	...
	public void processBatch() {
		...
		for (PrimGroupWriterInterface i : groups)
			i.addStringsToStringtable(getStringTable());

		stringtable.finish();
		// Now, start serializing.
		for (PrimGroupWriterInterface i : groups) {
			PrimitiveGroup group = i.serialize(this);
			if (group != null)
		...
		try {
			output.write(FileBlock.newInstance("OSMData",
	...
	/**
	 * Convert from a degrees represented as a double into the serialized
	 * offset.
	 */
